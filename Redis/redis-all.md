...
![正常缓存处理流程](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202201031022159.png)
...
![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202201031025371.png)
...
![try-redis](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202201040938066.png)
...
> Redis 基于 Reactor 模式开发了自己的网络事件处理器：这个处理器被称为**文件事件处理器**（file event handler）。文件事件处理器使用 I/O 多路复用（multiplexing）程序来同时监听多个套接字，并根据套接字目前执行的任务来为套接字关联不同的事件处理器。
>
...
![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202201040938443.png)
...
![redis4.0 more thread](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202201040938352.png)

不过，Redis 4.0 增加的多线程主要是针对一些**大键值对的删除操作的命令**，使用这些命令就会使用主处理之外的其他线程来“异步处理”。
...
Redis 通过一个叫做过期字典（可以看作是 hash 表）来保存数据过期的时间。过期字典的键指向 Redis 数据库中的某个 key(键)，过期字典的值是一个 long long 类型的整数，这个整数保存了 key 所指向的数据库键的**过期时间**（毫秒精度的 UNIX 时间戳）。

![redis过期字典](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202201040938856.png)
...
在 Redis 中通过更改 redis.conf 文件中的 hz 来改变定期删除函数的运行频率，默认每秒运行 10 次。

![img](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202202281726965.png)
...
可在 redis.conf 中通过 maxmemory <bytes\> 设置最大内存，不设定该参数默认是无限制的，但是通常会设定其为物理内存的四分之三

![image-20220228172834767](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202202281728819.png)
...
![redis事务](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202202221058991.png)
...
![redis roll back](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202202221058083.png)
...
![缓存穿透情况](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202202221058750.png)
...
![image](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture3/202207021245180.png)
...
然后，可能会出现这样一种情况：**不同的字符串可能哈希出来的位置相同。** （可以适当增加位数组大小或者调整我们的哈希函数来降低概率）
...
2、访问量大且更改频率不高的数据（读多写少）

最简单的缓存为 Map，本地缓存，这种缺点在于

1、缓存和项目在同一个环境中，当多个项目时，多个项目都需要这个缓存，那么其他项目将会无法访问这个缓存，所以其他项目将要访问数据库得到缓存。

2、当一个项目进行修改时，将会出现多个项目缓存一致性问题，类似于本地内存和主内存的关系

那么针对这两个问题，可使用 Redis 缓存中间件，并且针对 Redis 的缓存容量，可使用 Redis 集群
