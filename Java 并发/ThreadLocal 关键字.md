参考 https://juejin.cn/post/6844904151567040519

### 前言

![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301612990.png)
...
![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301612924.png)
...
![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301612228.png)
...
![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301612635.png)
...
![image.png](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301612099.png)

如果我们的**强引用**不存在的话，那么 `key` 就会被回收，也就是会出现我们 `value` 没被回收，`key` 被回收，导致 `value` 永远存在，出现内存泄漏。

### `ThreadLocal.set()`方法源码详解

![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301612459.png)
...
![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301612147.png)
...
![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301612659.png)
...
![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301612388.png)

这里直接将数据放到该槽位即可。

**第二种情况：** 槽位数据不为空，`key`值与当前`ThreadLocal`通过`hash`计算获取的`key`值一致：

![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301612115.png)

这里直接更新该槽位的数据。

**第三种情况：** 槽位数据不为空，往后遍历过程中，在找到`Entry`为`null`的槽位之前，没有遇到`key`过期的`Entry`：

![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301612972.png)
...
![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301612772.png)
...
以当前`staleSlot`开始 **向前迭代**查找，找其他过期的数据，然后更新过期数据起始扫描下标`slotToExpunge`。`for`循环迭代，直到碰到`Entry`为`null`结束。

如果找到了过期的数据，继续向前迭代，直到遇到`Entry=null`的槽位才停止迭代，如下图所示，**slotToExpunge被更新为0**：

![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301612044.png)
...
接着开始以`staleSlot`位置(index=7)向后迭代，**如果找到了和待插入 entry 相同key值的Entry数据：**

![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301612947.png)
...
![Yu4oWT.png](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112311507727.webp)


**向后遍历过程中，如果没有找到相同key值的Entry数据：**

![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301613619.png)
...
![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301613774.png)
...
![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301613935.png)
...
### `ThreadLocalMap`过期key的==探测式清理==流程
...
![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301613764.png)
...
![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301613372.png)
...
![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301613124.png)

我们假设`expungeStaleEntry(3)` 来调用此方法，如上图所示，我们可以看到`ThreadLocalMap`中`table`的数据情况，接着执行清理操作：

![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301613954.png)

第一步是清空当前`staleSlot`位置的数据，`index=3`位置的`Entry`变成了`null`。然后接着往后探测：

![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301613324.png)

执行完第二步后，index=4的元素挪到index=3的槽位中。

继续往后迭代检查，碰到正常数据，计算该数据位置是否偏移，如果被偏移，则重新计算`slot`位置，目的是让正常数据尽可能存放在正确位置或离正确位置更近的位置

![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301613165.png)
...
这里首先是会进行**探测式清理**工作，从`table`的起始位置往后清理，上面有分析清理的详细流程。清理完成之后，`table`中可能有一些`key`为`null`的`Entry`数据被清理掉，所以此时通过判断`size >= threshold - threshold / 4` 也就是`size >= threshold* 3/4` ，即 `len` 的 `1/2`，来决定是否扩容。

我们还记得上面进行`rehash()`的阈值是`size >= threshold`，所以当面试官套路我们`ThreadLocalMap`扩容机制的时候 我们一定要说清楚这两个步骤：

![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301613836.png)

接着看看具体的`resize()`方法，为了方便演示，我们以`oldTab.len=8`来举例：

![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301613530.png)
...
![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301613012.png)

**第二种情况：** `slot`位置中的`Entry.key`和要查找的`key`不一致：

![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301613572.png)

我们以`get(ThreadLocal1)`为例，通过`hash`计算后，正确的`slot`位置应该是4，而`index=4`的槽位已经有了数据，且`key`值不等于`ThreadLocal1`，所以需要继续**往后迭代**查找。
...
![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301613587.png)
...
### `ThreadLocalMap`过期key的==启发式清理==流程
...
![](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture2/202112301613227.png)

**「启发式清理」**，有这么一段注释，大概意思是；试探的扫描一些单元格，寻找过期元素。**当添加新元素或删除另一个过时元素时，将调用此函数。它执行对数扫描次数，作为不扫描（快速但保留垃圾）和与元素数量成比例的扫描次数之间的平衡，这将找到所有垃圾，但会导致一些插入花费 O(n) 时间。**
