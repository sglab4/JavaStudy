...
![enter image description here](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture3/202208061644984.png)
...
![enter image description here](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture3/202208061644938.png)
...
1. 使用者继承 AbstractQueuedSynchronizer 抽象类并重写指定的方法。（这些重写方法很简单，无非是对于共享资源 state 的获取和释放）
2. 将 AQS 组合在自定义同步组件的实现中，并调用其模板方法，而这些模板方法会调用使用者重写的方法。

上面说明的两点在 ReentrantLock 中得到体现，ReentrantLock 中的静态内部类 FairSync、NonfairSync 继承自 Sync，实现了抽象方法，然后 Sync 又继承自 AbstractQueuedSynchronizer，实现了 AbstractQueuedSynchronizer 的组件作为了自定义同步器的一部分。
