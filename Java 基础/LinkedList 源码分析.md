...
<font color="red">LinkedList</font>是一个实现了<font color="red">List接口</font>和<font color="red">Deque接口</font>的<font color="red">双端链表</font>。 **Queue**是简单的FIFO队列，**Deque**继承**Queue**实现双端队列。
...
```
上面可以看出addAll(int index, Collection c)方法通常包括下面四个步骤：
1. 检查index范围是否在size之内
2. toArray()方法把集合的数据存到对象数组中
3. 得到插入位置的前驱和后继节点
4. 遍历数据，将数据插入到指定位置
...
```
除了 add 方法外，offer 也可以增加元素，不过 offer 直接使用的就是 add 方法
...
```
remove 删除指定位置元素和删除指定元素都是使用 unlink 方法，链表的删除效率比 ArrayList 数组效率更高，是因为删除时，只会影响相邻元素的引用，而不会像数组那样需要移动其后所有的元素。
