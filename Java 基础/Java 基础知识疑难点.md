...
我们再看一下`java.util.AbstractList`的`remove()`方法，这样我们就明白为啥会抛出`UnsupportedOperationException`。因为 Arrays 的 ArrayList 类实现的是 AbstractList 类的方法，而 AbstractList 类的 add、remove 等操作直接抛出异常，因此不可添加删除元素，但**可使用 set 方法对元素进行修改**。
...
对于不可变集合，你可以使用[`ImmutableList`](https://github.com/google/guava/blob/master/guava/src/com/google/common/collect/ImmutableList.java)类及其[`of()`](https://github.com/google/guava/blob/master/guava/src/com/google/common/collect/ImmutableList.java#L101)与[`copyOf()`](https://github.com/google/guava/blob/master/guava/src/com/google/common/collect/ImmutableList.java#L225)工厂方法：（参数不能为空），如下，实现的 il 为不可变集合，不可变意味着不能做任何修改，不能添加删除，也不能使用 set 修改某个元素，只能读取集合中的内容。

```java
List<String> il = ImmutableList.of("string", "elements");  // from varargs
List<String> il = ImmutableList.copyOf(aStringArray);      // from array
```
对于可变集合，你可以使用[`Lists`](https://github.com/google/guava/blob/master/guava/src/com/google/common/collect/Lists.java)类及其[`newArrayList()`](https://github.com/google/guava/blob/master/guava/src/com/google/common/collect/Lists.java#L87)工厂方法，如下创建出的集合和普通的 new ArrayList 创建的集合相同，可添加删除等操作。
