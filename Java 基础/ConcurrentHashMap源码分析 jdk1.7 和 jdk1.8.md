...
![Java 7 ConcurrentHashMap 存储结构](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture3/202208061519974.png)
...
ConcurrentHashMap 的扩容只会扩容到原来的**两倍**。老数组里的数据移动到新的数组时，位置要么不变，要么变为 index+ oldSize，参数里的 node 会在扩容之后使用链表**头插法**插入到指定位置。
...
![Java8 ConcurrentHashMap 存储结构（图片来自 javadoop）](https://pict-picgo.oss-cn-hangzhou.aliyuncs.com/picture3/202209222019030.png)
...
5. 如果当前位置有元素，则利用 synchronized 锁写入数据。
...
为什么不加锁就可以实现 get，这是因为在 Node 中，定义 V val 为 volatile，即使在并发下，也可以读到最新值。
