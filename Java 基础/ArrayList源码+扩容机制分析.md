...
//那么频繁的插入会导致频繁的拷贝，降低性能，而ArrayList的扩容机制避免了这种情况。
  ...
  //判断是否需要扩容
    ...
    public Object[] toArray() {
        ...
        /**
         * 若 toArray 给定了参数，则需要和 List 中存储的数据类型相同，传入的数组大小分为两种情况：
         * 1、若数组长度小于集合长度，则返回一个新的数组使得和 List 中元素长度相同
         * 2、若数组长度大于集合长度，则原数组中多于的部分将会赋值为 null
         */
...
ArraysList 的 toArray 方法调用了 Arrays.copyOf() 方法，而 Arrays.copyOf() 又调用了 System.arraycopy() 方法。
...
ArrayList 源码中有一个 `ensureCapacity` 方法不知道大家注意到没有，这个方法 ArrayList 内部没有被调用过，所以很显然是提供给用户调用的，那么这个方法有什么作用呢？指定 ArrayList 的最小容量，在 add 大量元素之前使用，减小 grow 重新分配的次数。
