...
![](https://gitee.com/sgkurisu/pic-go/raw/master/picture2/202112201034262.png)
...
5. **底层数据结构：** JDK1.8 以后的 `HashMap` 在解决哈希冲突时有了较大的变化，当链表长度大于阈值（默认为 8）（将链表转换成红黑树前会判断，如果当前数组的长度小于 64，那么会选择先进行数组扩容，而不是转换为红黑树）时，将链表转化为红黑树，以减少搜索时间。Hashtable 直接采用的是数组加链表的机制。
...
3. 两个对象有相同的 `hashcode` 值，它们也不一定是相等的。也就是说，两个不相同对象的 hashcode 可能是相同的。
...
### 1.4.11 为什么 JDK1.8 HashMap 中 hash 的计算方法中 hashCode 需要无符号右移 16 位？
...
**Collections 中方法是对集合进行操作，而 Arrays 中则是对数组进行操作。**
