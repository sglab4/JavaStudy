...
- User-Agent：服务器得到浏览器的版本信息，因此服务器可在服务端解决浏览器兼容问题
- Referer：浏览器得到本次访问路径 url
  - 作用：防盗链、统计访问信息
...
- Content-Type：本次响应体的数据格式及编码格式
- Content-disposition：本次响应以什么方式打开响应体的数据
  - in-line：默认，在当前页面打开
...
上面提到对称密钥加密方式的传输效率更高，但是无法安全地将密钥 Secret Key 传输给通信方。而非对称密钥加密方式可以保证传输的安全性，因此我们可以利用非对称密钥加密方式将 Secret Key  传输给通信方。HTTPS 采用**混合的加密机制**，正是利用了上面提到的方案：（使用非对称密钥加密的方式将对称密钥加密的密钥进行安全传输，然后再使用对称密钥加密的方式保证安全性）

1. 使用非对称密钥加密方式，传输对称密钥加密方式所需要的 Secret Key，从而保证安全性;
2. 获取到 Secret Key 后，再使用对称密钥加密方式进行通信，从而保证效率。（下图中的 Session Key 就是 Secret Key）
...
注：1.DNS域名解析采用的是**递归查询**的方式，过程是，先去找DNS缓存->缓存找不到就去找根域名服务器->根域名又会去找下一级，这样递归查找之后，找到了，给我们的web浏览器
